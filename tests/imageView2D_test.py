...
class TestImageRendering:
    ...
    def test_renders_correctly_unzoomed_in_viewport_larger_than_image(
        ...
        extra_width = int(margin_factor * image_width)
        extra_height = int(margin_factor * image_height)
        padding_width = (image_view_width - image_width) // 2
        padding_height = (image_view_height - image_height) // 2
        numpy.testing.assert_array_equal(
            rendered[
                padding_height + extra_height : (image_height + padding_height - extra_height - 1),
                padding_width + extra_width : (image_width + padding_width - extra_width - 1),
                0,
            ],
    ...
    def test_random_image_render_scaling(
        ...
        """
        Test if zoomed in image is rendered correctly
        """
        image_view.show()
        ...
        assert zoom > 1, "The simple striding method for accessing zoomed in values only works if zoom is >1"
        rendered_resampled = rendered[::zoom, ::zoom, 0]
