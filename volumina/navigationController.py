...
class NavigationInterpreter(QObject):
    ...
    def eventFilter(self, watched, event):
        ...
        if self._current_state == self.DEFAULT_MODE:
            ...
            if (etype == QEvent.MouseButtonPress and event.button() == Qt.MidButton) or (
                etype == QEvent.MouseButtonPress and event.modifiers() == Qt.ShiftModifier
            ):
    ...
    def _itemsAt(self, imageview, pos):
        ...
        itms = [
            ...
            for x in itms
            if not (
                isinstance(x, volumina.sliceIntersectionMarker.SliceIntersectionMarker)
                or isinstance(x, volumina.imageScene2D.DirtyIndicator)
                or isinstance(x, volumina.crossHairCursor.CrossHairCursor)
                or x is imageview.scene().dataRectItem
            )
        ]
        return itms
...
class NavigationController(QObject):
    ...
    def changeChannel(self, newChannel):
        ...
        for pump in self._imagePumps:
            if 2 not in pump.syncedSliceSources.getSyncAlong():
                raise RuntimeError(
                    "NavigationController.changeChannel: channel axis not synced in all image pumps; can't apply method"
                )
    ...
    def _updateSlice(self, num, axis):
        if num < 0 or num >= self._model.volumeExtent(axis):
            raise Exception(
                "NavigationController._setSlice(): invalid slice number = %d not in range [0,%d)"
                % (num, self._model.volumeExtent(axis))
            )
